 ...
 */
function send_password_mail(string $email, string $name, string $token): bool
{
    global $CONFIG;
    $base = (string)($CONFIG['verify_url'] ?? '');
    $verifyUrl = rtrim($base, '?&') . (str_contains($base, '?') ? '&' : '?') . 'token=' . $token;
    return send_html_mail($email, '【Looper】メールアドレスの確認', password_mail_body($name, $verifyUrl));
}

/**
 * パスワード設定メールの本文（黒帯＋公式ロゴ。現行メールと同じ意匠）。
 */
function password_mail_body(string $memberName, string $verifyUrl): string
{
    ...
    return
        ...
        '<a href="' . $url . '" style="background:#C0281C;color:white;padding:14px 32px;border-radius:8px;text-decoration:none;font-size:15px;font-weight:700;display:inline-block;">パスワードを設定する</a>' .
        '</div>' .
