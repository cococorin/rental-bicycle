 ...
 *    - 予約の管理（空き確認・追加・キャンセル）
 *    - 利用記録の管理（現地受付）
 *    - 設定管理（営業時間・定休日・料金・バッファ）
 *    - ログイン・パスワード設定 → はんだのたねGASへリレー
 *    - 会員検索 → はんだのたねGASへリレー
 *    - メール認証トークン検証ページの表示
 *
 *  ★ このGASは ikewaki.keita@handanotane.com アカウントで作成・デプロイしてください
 *
 *  【スプレッドシート構成（ikewakiアカウント）】
 *    「予約」シート
 *      A: 予約番号  B: 会員番号  C: 氏名  D: 自転車ID  E: 日付
 *      F: 開始時刻  G: 終了時刻  H: ステータス  I: コース
 *      J: 前払額  K: メモ  L: 予約日時
 *    「利用記録」シート
 *      A: 取引番号  B: 会員番号  C: 氏名  D: 車種  E: コース
 *      F: ヘルメット  G: ロッカー  H: 開始日時  I: 返却予定
 *      J: ステータス  K: 前払額  L: 追加精算  M: 返却日時  N: メモ
 *    「設定」シート
 *      A: 設定キー  B: 値  C: 説明
 * ============================================================
 */
...
//  【リレー処理】はんだのたねGASへ転送
// ============================================================
function relayGet(actionParam) {
  if (!MEMBER_API_URL || MEMBER_API_URL === 'HANDANOTANE_GAS_URL_HERE') {
    return jsonResponse({ error: 'MEMBER_API_URL が設定されていません' });
  }
  try {
    ...
    return jsonResponse({ error: 'はんだのたねGAS接続エラー: ' + err.message });
  }
}

function relayPost(action, body) {
  if (!MEMBER_API_URL || MEMBER_API_URL === 'HANDANOTANE_GAS_URL_HERE') {
    return jsonResponse({ error: 'MEMBER_API_URL が設定されていません' });
  }
  try {
...
// ============================================================
function getBookings(fromStr, toStr) {
  var sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_BOOKINGS);
...
function addBooking(body) {
  ...
  if (tMin(body.startTime) < tMin(settings.openTime) || tMin(body.endTime) > tMin(settings.closeTime)) {
    return jsonResponse({ success: false, error: '営業時間外です（' + settings.openTime + '〜' + settings.closeTime + '）' });
  }
  if (tMin(body.startTime) >= tMin(body.endTime)) {
    return jsonResponse({ success: false, error: '終了時刻は開始時刻より後に設定してください' });
  }
  var conflict = checkConflict(body.bikeId, body.date, body.startTime, body.endTime, settings.bufferMinutes);
  if (conflict) {
    return jsonResponse({ success: false, error: 'この時間帯は予約済みです（' + conflict.start + '〜' + conflict.bufferEnd + ' 清掃バッファ含む）' });
  }
...
// ============================================================
//  【予約キャンセル】POST ?action=cancelBooking
// ============================================================
function cancelBooking(body) {
  if (!body.bookingId) return jsonResponse({ success: false, error: 'bookingId required' });
  var sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_BOOKINGS);
  if (!sheet) return jsonResponse({ success: false, error: 'sheet not found' });
  var data = sheet.getDataRange().getValues();
  for (var i = 1; i < data.length; i++) {
...
function setupSheets() {
  ...
  if (!cfg.getRange('A1').getValue()) {
    ...
    cfg.getRange('A2:C11').setValues([
      ...
      ['closedDays',    '2',      '定休日（0=日〜6=土、複数は「0,2」）'],
